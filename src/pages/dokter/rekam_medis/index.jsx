...
export default function RekamMedisPasien() {
  ...
  // Jika admin yang periksa, kita samarkan jadi dokter
  const getPemeriksaName = (user) => {
    if (!user) return "—";
    if (user.role === "admin") {
      // Tambahkan gelar dr. jika belum ada
      return user.username.toLowerCase().startsWith("dr") ? user.username : `dr. ${user.username}`;
    }
  ...
  const getPemeriksaRole = (user) => {
    if (!user) return "—";
    return user.role === "admin" ? "dokter" : user.role;
  };
  ...
  const handleExportPDF = (filteredData) => {
    try {
      ...
      doc.setFontSize(10);
      doc.text("Praktek Dokter Umum dr. Rowi", 14, 22);
  ...
  const handleExportExcel = (filteredData) => {
    try {
      ...
      const dataToExport = filteredData.map((p, index) => ({
        ...
        "Dokter Pemeriksa": getPemeriksaName(p.user), // ILUSI
        "Keluhan": p.keluhan ?? "-",
        "Diagnosa": p.diagnosa ?? "-",
    ...
    filteredData.forEach((p, index) => {
      tableRows += `
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #ddd;">${getPemeriksaName(p.user)}</td> <!-- ILUSI -->
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5">
                          <span className="text-xs text-gray-500 dark:text-gray-400">
                            Pemeriksa:&nbsp;
                            {/* ILUSI */}
                            <span className="font-medium text-gray-700 dark:text-gray-300">{getPemeriksaName(p.user)}</span>
                          </span>
                    ...
                    <div className="border-t border-gray-100 dark:border-gray-700">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-3 bg-gray-50 dark:bg-gray-700/20 border-t border-gray-100 dark:border-gray-700">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            {/* ILUSI NAMA & ROLE */}
                            <p className="text-xs font-semibold text-gray-900 dark:text-white">{getPemeriksaName(p.user)}</p>
                            <p className="text-xs text-gray-400 dark:text-gray-500 capitalize">{getPemeriksaRole(p.user)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
