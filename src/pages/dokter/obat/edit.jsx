...
import { showObat, updateObat } from "../../../_sevices/obat";


export default function EditObatDok() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const ObatData = await showObat(id);
        setFormdata({
          nama_obat: ObatData.data.nama_obat,
          harga_obat: ObatData.data.harga_obat,
        });
        setIsLoading(false);
      } catch (error) {
        console.error("Error fetching data:", error);
        alert("Gagal memuat data");
        navigate("/dokter/obat");
      }
    };
    fetchData();
  }, [id, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submitData.append("nama_obat", formData.nama_obat);
      submitData.append("harga_obat", formData.harga_obat);
      ...
      await updateObat(id, submitData);
      navigate("/dokter/obat");
    } catch (error) {
    ...
    <section className="bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="max-w-2xl px-4 py-6 mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">Form Edit Obat</h2>
          </div>
          <button
            onClick={() => navigate("/dokter/obat")}
                ...
                <div className="flex gap-3">
                  <div>
                    ...
                    <p className="text-sm text-center text-green-800 dark:text-green-400">
                      Anda sedang mengedit obat. Pastikan semua informasi benar sebelum menyimpan perubahan.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">Nama obat</p>
                  </div>
                </div>
                <input
                  type="text"
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">Harga obat</p>
                  </div>
                </div>
                <input
                  type="number"
